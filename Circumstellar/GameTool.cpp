...
bool GameTool::ChangeTerrain(Planet* planet, Camera* currentCam, float changePower) {
	...
	//float arrayRadiusCorner = std::sqrt(std::pow(x, 2) + std::pow(y, 2) + std::pow(z, 2));
	//msg = L"Distance to corner: " + std::to_wstring(arrayRadiusCorner);  msg += L"\n"; OutputDebugString(msg.c_str());
	//If close enough, do the adjustment loop.
