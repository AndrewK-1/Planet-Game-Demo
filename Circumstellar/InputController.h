...
struct InputBinding {
	UINT key; //Key to press
	Action action; //Action or method that is called
};

class InputController {
public:
	InputController();
	UINT GetGameBindSetting(UINT settingId); //Get the key value of the specific action.
	void RemoveAllPressedKeys();
	void BindGameKey(UINT key, Action action);
	void BindMenuKey(UINT key, Action action);
	void SetGameBindSetting(UINT oldKey, UINT newKey, UINT settingID);
	...
	void PrepareKeybindChange(int buttonIndex, UINT settingID);
...
private:
	...
	bool m_keyChangeInputContextEnabled;
	UINT m_nextKeyToChangeSettingID;
	int m_buttonChangeIndex; //Index of button to be changed
	float m_cameraSpeed;
	float m_rollSpeed;
